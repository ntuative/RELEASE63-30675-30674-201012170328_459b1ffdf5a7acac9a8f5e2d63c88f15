package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1405:int = 4;
      ...
      {
         return this.var_359.length + this.var_253.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_253.length < const_1405 && this.var_359.length > 0)
            {
               _loc1_ = this.var_359.shift();
               this.var_253.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_359.indexOf(loader);
            if(index > -1)
            {
               this.var_359.splice(index,1);
            }
            index = this.var_253.indexOf(loader);
            if(index > -1)
            {
               this.var_253.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_359)
            {
               if(param2)
               {
