package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1039:IContainerButtonWindow;
      
      private var var_1040:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1630 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1040 = this.var_1630.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1040)
         {
            this.var_1040.procedure = this.onButtonClick;
         }
         this.var_1039 = this.var_1630.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1039)
         {
            this.var_1039.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_234 >= this._localizations.length - 1)
         {
            this.var_1039.disable();
         }
         else
         {
            this.var_1039.enable();
         }
         if(this.var_234 <= 0)
         {
            this.var_1040.disable();
         }
         else
         {
            this.var_1040.enable();
         }
      }
      ...
      {
         ++this.var_234;
         this.var_234 = Math.min(this.var_234,this._localizations.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_234;
         this.var_234 = Math.max(this.var_234,0);
         this.openPage();
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this._localizations[this.var_234][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this._localizations[this.var_234][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this._localizations[this.var_234][1]).value;
