package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private var var_153:IWidgetAvatarEffect;
      
      private var var_799:IWindow;
      ...
      {
         super();
         this.var_181 = new Timer(1000);
         this.var_181.addEventListener(TimerEvent.TIMER,this.onUpdate);
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this.var_181.stop();
            this.var_181.removeEventListener(TimerEvent.TIMER,this.onUpdate);
            this.var_181 = null;
         }
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_153.isActive)
            {
               _loc2_ = this.var_153.secondsLeft / Number(this.var_153.duration);
               this.var_799.width = _loc2_ * this.var_487;
            }
            else
            {
               this.var_799.width = 0;
               this.var_181.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_800.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_800.text;
            if(this.var_153.isActive)
            {
               _loc2_ = this.var_153.secondsLeft;
               ...
               this.var_800.text = _loc1_;
            }
            else
            {
               this.var_800.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_153.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_153.isActive)
         {
         ...
         else
         {
            ...
            this.var_801 = this._window.findChildByName("effect_hilite");
            this.var_801.visible = false;
         }
         this.var_800 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_799 = this._window.findChildByName("loader_bar");
         if(this.var_799 != null)
         {
            this.var_487 = this.var_799.width;
            this.var_181.start();
            this.onUpdate();
         }
         if(this.var_153.icon)
         {
            this.setElementBitmap("effect_icon",this.var_153.icon);
         }
      }
