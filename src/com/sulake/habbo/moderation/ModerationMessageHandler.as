package com.sulake.habbo.moderation
{
   ...
   public class ModerationMessageHandler
   {
      ...
      {
         if(param1 == null || this.var_60 == null)
         {
            return;
         }
         ...
         this.var_60.issueManager.playSound(_loc3_);
         this.var_60.issueManager.updateIssue(_loc3_);
         this.var_60.issueManager.updatePriorities();
      }
      ...
      private function onModeratorInit(param1:ModeratorInitMessageEvent) : void
      {
         ...
         {
            this.var_60.issueManager.updateIssue(_loc6_);
         }
         this.var_60.issueManager.updatePriorities();
         this.var_60.initMsg = _loc3_;
         this.var_60.startPanel.show();
      }
      ...
      private function onIssuePickFailed(param1:IssuePickFailedMessageEvent) : void
      {
         ...
         var pickedAlready:Boolean = this.var_60.issueManager.issuePickFailed(issues);
         if(pickedAlready)
         {
            if(retryEnabled)
            {
               ...
               {
                  alert = false;
                  this.var_60.issueManager.autoPick(retryEnabled,retryCount);
               }
            }
         }
         if(alert)
         {
            this.var_60.windowManager.alert("Error","Issue picking failed",0,function(param1:IAlertDialog, param2:Event):void
            {
               param1.dispose();
            });
         }
      ...
      private function onIssueDeleted(param1:IssueDeletedMessageEvent) : void
      {
         ...
         this.var_60.issueManager.removeIssue(_loc2_.issueId);
      }
      ...
      private function onUserInfo(param1:ModeratorUserInfoEvent) : void
      {
         ...
         for each(_loc3_ in this.var_1020)
         {
      ...
      private function onRoomInfo(param1:ModeratorRoomInfoEvent) : void
      {
         ...
         for each(_loc3_ in this.var_1019)
         {
      ...
      private function onCfhChatlog(param1:CfhChatlogEvent) : void
      {
         ...
         this.onChatlog("Call For Help Chatlog: " + _loc2_.data.chatRecordId,WindowTracker.const_949,_loc2_.data.callId,_loc3_,_loc4_);
      }
      ...
      private function onRoomChatlog(param1:RoomChatlogEvent) : void
      {
         ...
         var _loc4_:Dictionary = new Dictionary();
         this.onChatlog("Room Chatlog: " + _loc2_.data.roomName,WindowTracker.const_574,_loc2_.data.roomId,_loc3_,_loc4_);
      }
      ...
      private function onUserChatlog(param1:UserChatlogEvent) : void
      {
         ...
         this.onChatlog("User Chatlog: " + _loc2_.data.userName,WindowTracker.const_905,_loc2_.data.userId,_loc2_.data.rooms,_loc3_);
      }
      
      private function onChatlog(param1:String, param2:int, param3:int, param4:Array, param5:Dictionary) : void
      {
         var _loc6_:* = null;
         for each(_loc6_ in this.var_1021)
         {
      ...
      private function onRoomEnter(param1:RoomEntryInfoMessageEvent) : void
      {
         ...
         for each(_loc3_ in this.var_821)
         {
            _loc3_.onRoomChange();
         }
      }
      ...
      private function onModeratorActionResult(param1:ModeratorActionResultMessageEvent) : void
      {
         ...
         if(_loc2_.success)
         {
            this.var_60.connection.send(new GetModeratorUserInfoMessageComposer(_loc2_.userId));
         }
         else
         {
            this.var_60.windowManager.alert("Alert","Moderation action failed",0,this.onAlertClose);
         }
      }
