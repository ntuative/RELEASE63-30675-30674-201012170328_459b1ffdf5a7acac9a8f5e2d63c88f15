package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_654:IFigurePartSet;
      ...
      private var var_542:Rectangle;
      
      private var var_1514:int = 0;
      
      private var var_363:IAvatarRenderManager;
      
      private var var_689:Boolean;
      
      private var var_1513:BitmapData;
      
      private const const_1824:int = 13421772;
      
      private const const_1825:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_623 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1514 = Math.max(this.var_1514,_loc6_.colorLayerIndex);
            }
         }
         this.var_363 = this.var_63.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_623 != null && !this.var_1515)
            {
               _loc3_ = this.var_623;
            }
            else
            ...
            if(this.var_654)
            {
               switch(this.var_654.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_53:
                     _loc2_.style = HabboIconType.const_591;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:AvatarFigureContainer = this.var_363.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_363.isFigureReady(_loc1_))
         {
            this.var_363.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_311[_loc6_] + "_" + FigureData.const_132;
               _loc4_ = this.var_363.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_542 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1513)
               {
                  _loc9_ = this.var_63.controller.assets.getAssetByName("avatar_editor_download_icon");
                  this.var_1513 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_542.width,this.var_542.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_542.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_542.y;
               if(this.var_1515 && _loc2_.colorLayerIndex > 0)
               {
