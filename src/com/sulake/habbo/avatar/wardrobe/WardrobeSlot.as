package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_330:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_330 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_598 && this.var_1358)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_81,this.var_874,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_32,parseInt(FigureData.const_801));
         ...
         if(this.var_330)
         {
            if(this.var_330.bitmap)
            {
               this.var_330.bitmap.dispose();
            }
            this.var_330.bitmap = new BitmapData(this.var_330.width,this.var_330.height,true,0);
            _loc7_ = (this.var_330.width - _loc1_.width) / 2;
            _loc8_ = (this.var_330.height - _loc1_.height) / 2;
            this.var_330.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
         ...
         if(_loc4_)
         {
            _loc4_.visible = this.var_1358 && this.var_598 != null;
         }
      }
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_598 = this._controller.figureData.getFigureString();
               this.var_874 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1772,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_598)
               {
                  this._controller.loadAvatarInEditor(this.var_598,this.var_874,this._controller.clubMemberLevel);
               }
         }
      }
